...
if(!user) {
      ...
      if(user.roles.has(role.id)) {
            message.channel.send({embed: {description: `${user.user.tag} already has <@&${role.id}>`, color: EmbedColor}})
      } else {
            user.addRole(role.id).then(f => {
            message.channel.send({embed: {description: `${user.user.tag} was given <@&${role.id}>`, color: EmbedColor}})
